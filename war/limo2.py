...
def get_road_names(point):
    ...
    names = findAllIntersectionRoads((point[0], point[1]), 0.003)
...
#move to the next intersection within specified distance
def move_next_intersection(current_street, delimeter, commuter):
    ...
    # nextPoint = move_distance(current_street, delimeter, testCommuter)
    # print "nextPoint:"
    # print nextPoint
    # newRoads = findAllIntersectionRoads2(testCommuter[len(commuter)-1:])

    newRoads = findAllIntersectionRoads(currentPoint, delimeter)


    newRoadName = getNewRoads(initialRoads, newRoads, currentPoint, current_street, testCommuter)
...
def getNewRoads(initialRoads, newRoads, currentPoint, current_street, tcommuter):
    found = False
    newRoadNames = []
    global bearing
    previousBearing = bearing
...
def move_distance(street, distance, commuter):
        ...
        if not verify_street_point(street, nextLonLat) :
                #return
                raise Exception("Cannot move for " +str(distance)+ " on " + street)

        pnts = get_sub_road(street, ZIP, get_EWKT(commuter[len(commuter)-1]), get_EWKT(nextLonLat))
        ...
        if pnts and len(pnts[1]) == 2:
            ...
            last_dist = distFrom(float(last_point[0]), float(last_point[1]), float(nextLonLat[0]), float(nextLonLat[1]))
            ...
            p3 = polarToCartesian(nextLonLat)
